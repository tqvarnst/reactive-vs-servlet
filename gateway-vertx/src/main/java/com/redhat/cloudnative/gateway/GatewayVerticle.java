...
public class GatewayVerticle extends AbstractVerticle {
        ...
        ServiceDiscovery.create(vertx, discovery -> {
            ...
            Single<WebClient> catalogDiscoveryRequest = HttpEndpoint.rxGetWebClient(discovery,
                    ...
                    .onErrorReturn(t -> WebClient.create(vertx, new WebClientOptions()
                            .setDefaultHost(System.getProperty("catalog.api.host", "catalog-coolstore-vertx.192.168.64.13.nip.io"))
                            .setDefaultPort(Integer.getInteger("catalog.api.port", 80))));

            // Inventory lookup
            Single<WebClient> inventoryDiscoveryRequest = HttpEndpoint.rxGetWebClient(discovery,
                    rec -> rec.getName().equals("inventory"))
                    .onErrorReturn(t -> WebClient.create(vertx, new WebClientOptions()
                            .setDefaultHost(System.getProperty("inventory.api.host", "inventory-coolstore-vertx.192.168.64.13.nip.io"))
                            .setDefaultPort(Integer.getInteger("inventory.api.port", 80))));
    ...
    private void products(RoutingContext rc) {
        // Retrieve catalog
        catalog.get("/api/catalog.json").as(BodyCodec.jsonArray()).rxSend()
                .map(resp -> {
                ...
                .flatMap(products ->
                        ...
                        Observable.from(products)
                                ...
                                .flatMapSingle(product ->
                                        inventory.get("/api/inventory.json").as(BodyCodec.jsonObject())
                                                .rxSend()
                                                .map(resp -> {
                                                    if (resp.statusCode() != 200) {
